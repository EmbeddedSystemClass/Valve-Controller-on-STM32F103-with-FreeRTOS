...
void vTaskZmReceiver(void *pvParameters)
{
	...
	while (1)
	{
		...
		case ZWAVE_IDLE:
		{
			...
			if (xStatus == pdPASS)
			{
				...
				if (swData.swTaskSource == xBTN2)
				{
					if (swData.swData == LONG_PRESS)
					{
						...
						//uint8_t data;
						/*---------add uiset-----------*/
						//							uiMemSet(FUNC_WRITE_USER0_ADR,01,DOWNLINK_ZW);
						//							uiMemSet(FUNC_WRITE_USER1_ADR,01,DOWNLINK_ZW);
						//							uiMemSet(FUNC_WRITE_USER2_ADR,01,DOWNLINK_ZW);
						//							uiMemSet(FUNC_WRITE_USER3_ADR,01,DOWNLINK_ZW);
						xStatus = xQueueSend(pxValveHandles->xQueueControlDownlink, &data_control, xTicksToWait);
						if (xStatus != pdPASS)
						{
				...
				if (swData.swTaskSource == xBTN3)
				{
					if (swData.swData == LONG_PRESS)
					{
						...
						data_control.cmdSource = ZWAVE;
						//							uiMemSet(FUNC_WRITE_USER0_ADR,00,DOWNLINK_ZW);
						//							uiMemSet(FUNC_WRITE_USER1_ADR,00,DOWNLINK_ZW);
						//							uiMemSet(FUNC_WRITE_USER2_ADR,00,DOWNLINK_ZW);
						//							uiMemSet(FUNC_WRITE_USER3_ADR,00,DOWNLINK_ZW);
						//uint8_t data;
			...
			//uiMemGet(FUNC_WRITE_USER0_ADR, &linkType);
			//if (linkType == UPLINK_ZW)
			//xQueuePeek
			xStatus = xQueuePeek(pxValveHandles->xQueueReponseUplink, &data_reponse, NULL);
			if (xStatus == pdPASS)
			{
				xStatus = pdFAIL;
				if (data_reponse.cmdSource == SHARED_MEM)
				{
					xStatus = xQueueReceive(pxValveHandles->xQueueReponseUplink, &data_reponse, NULL);
