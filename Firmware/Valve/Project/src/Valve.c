...
void vTaskControlMotor(void *pvParameters)
{
	...
	while (TRUE)
	{
		...
		xStatus = xQueuePeek(pxValveHandles->xQueueControlDownlink, &data_control, NULL);

		if (xStatus == pdPASS)
		{
			xStatus = pdFAIL;
			if (data_control.cmdSource == SHARED_MEM)
			{
				xStatus = xQueueReceive(pxValveHandles->xQueueControlDownlink, &data_control, NULL);
				if (xStatus == pdPASS)
				{
		...
		for (uint8_t motor_num = 0; motor_num < MAX_MOTOR; motor_num++)
		{
			...
			case MOTOR_TYPE_0:
			{
				if (MotorState[motor_num] == MOTOR_RUN_LEFT)
				{
					...
					data.state = 0x00;
					xQueueSend(pxValveHandles->xQueueReponseUplink, &data, NULL);
					MotorState[motor_num] = MOTOR_AT_LEFT;
