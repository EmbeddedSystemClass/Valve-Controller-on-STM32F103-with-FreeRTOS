...
static void UARTs_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(MB_UART_CLK, ENABLE);
  ...
  GPIO_InitStructure.GPIO_Pin = MB_UART_RxPin;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
  GPIO_Init(MB_UART_GPIO, &GPIO_InitStructure);
  /* Configure DBG Tx as alternate function push-pull */
  GPIO_InitStructure.GPIO_Pin = MB_UART_TxPin;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
  GPIO_Init(MB_UART_GPIO, &GPIO_InitStructure);

  USART_InitStructure3.USART_BaudRate = 9600;
  ...
  USART_Init(MB_UART, &USART_InitStructure2);
  /* Enable DBG_UART Receive interrupts */
  USART_ITConfig(MB_UART, USART_IT_RXNE, ENABLE);
  /* Enable DBG_UART   */
  USART_Cmd(MB_UART, ENABLE);
	/* Config UARTRDE*/
	  GPIO_InitStructure.GPIO_Pin = MB_UART_RDE;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
